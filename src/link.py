...
# TODO print out the g++ commands
# TODO is there a cleaner way to provide o_changed flag

def link_o(cpp_version: int, shared: bool, static: bool, \
           o_paths: list, lib_names: list, lib_dir_paths: list, target_path: str) -> None:
    ...
    command += " -o " + target_path + " "
    command += ' '.join(o_paths) + " "
    command += ' '.join([f'-l{lib_name}' for lib_name in lib_names]) + " "
    command += ' '.join([f'-L{lib_dir_path}' for lib_dir_path in lib_dir_paths]) + " "
...
def link(cpp_version: int, shared: bool, static: bool, \
         lib_names: list, lib_dir_paths: list, build_dir_path: str, target_path: str, o_changed = False) -> None:
    # Get current o files
    ...
    if o_changed or not utils.path_exists(target_path):
        link_o(cpp_version, shared, static, o_paths, lib_names, lib_dir_paths, target_path)
        return
